...
let appData = {
    ...
    asking: function () {
        ...
        let addExpenses;
        do {
            addExpenses = prompt ('Перечислите возможные расходы за рассчитываемый период через запятую', 'машина, квартира, Cтудия');
        }
        ...
        for (let i = 0; i < 2; i++) {
            ...
            } while (isNumber(exp) || exp.trim() === '' || exp === null);
            
            do {
                amount = prompt('Во сколько это обойдется?', 5000);
            } while (!isNumber(amount) || amount.trim() === '' || amount === null);
